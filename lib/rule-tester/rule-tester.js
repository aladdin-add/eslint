...
/* eslint no-use-before-define: [2, {"functions": false}] -- functions are hoisted */
//------------------------------------------------------------------------------
// Requirements
//------------------------------------------------------------------------------
...
class RuleTester {
    ...
    run(ruleName, rule, test) {
        ...
        function assertTest() {
            ...
            /**
             * Asserts that the given (valid/invalid) test case is valid.
             * @param {Object} testcase The test case to assert.
             * @param {["valid" | "invalid"]} scenarioType The type of the test case.
             * @returns {void}
             */
            function assertTestCase(testcase, scenarioType) {
                assert.ok(testcase && typeof testcase === "object", "Test case must be a string or object");
                assert.ok(typeof testcase.code === "string", "Test case must specify a string value for 'code'");

                if (hasOwnProperty(testcase, "name")) {
                    assert.ok(typeof testcase.name === "string", "Optional test case property 'name' must be a string");
                }
                if (hasOwnProperty(testcase, "only")) {
                    assert.ok(typeof testcase.only === "boolean", "Optional test case property 'only' must be a boolean");
                }
                if (hasOwnProperty(testcase, "filename")) {
                    assert.ok(typeof testcase.filename === "string", "Optional test case property 'filename' must be a string");
                }
                if (hasOwnProperty(testcase, "options")) {
                    assert(Array.isArray(testcase.options), "options must be an array");
                }

                if (hasOwnProperty(testcase, "languageOptions")) {
                    const parser = testcase?.languageOptions?.parser;

                    if (parser && typeof parser !== "object") {
                        assert.fail("Parser must be an object with a parse() or parseForESLint() method.");
                    }
                }

                if (scenarioType === "valid") {
                    if (hasOwnProperty(testcase, "errors")) {
                        if (typeof testcase.errors === "number") {
                            assert.ok(testcase.errors === 0, "Valid test cases should not have an 'errors' property");
                        } else if (Array.isArray(testcase.errors)) {
                            assert.ok(testcase.errors.length === 0, "Valid test cases should not have an 'errors' property");
                        } else {
                            assert.fail("Valid test cases should not have an 'errors' property");
                        }
                    }

                    assert.ok(testcase.output === null || testcase.output === void 0, "Valid test case should not have a property 'output'");
                }

                if (scenarioType === "invalid") {
                    assert.ok(hasOwnProperty(testcase, "errors"), `Did not specify errors for an invalid test of ${ruleName}`);
                    assert.ok(testcase.output === null || testcase.output === void 0 || typeof testcase.output === "string", "Invalid test case property 'output' must be a string or null");
                    if (typeof testcase.errors === "number") {
                        assert.ok(testcase.errors > 0, "Invalid cases must have 'error' value greater than 0");
                    } else if (Array.isArray(testcase.errors)) {
                        assert.ok(testcase.errors.length > 0, "Invalid cases must have at least one error");
                        for (let i = 0; i < testcase.errors.length; i++) {
                            if (typeof testcase.errors[i] === "string" || testcase.errors[i] instanceof RegExp) {
                                testcase.errors[i] = { message: testcase.errors[i] };
                            }

                            const error = testcase.errors[i];

                            if (error && typeof error === "object") {
                                if (hasOwnProperty(error, "message")) {
                                    assert.ok((typeof error.message === "string" && error.message !== "") || (error.message instanceof RegExp), "Error message must be a non-empty string or a RegExp");
                                }
                                if (hasOwnProperty(error, "messageId")) {
                                    ...
                                    assert.ok(typeof error.messageId === "string" && error.messageId !== "", "Error messageId must be a non-empty string");
                                    if (!hasOwnProperty(rule.meta.messages, error.messageId)) {
                                        assert.fail(`Invalid messageId '${error.messageId}'. Expected one of ${friendlyIDList}.`);
                                    }
                                }
                                if (hasOwnProperty(error, "data")) {
                                    assert.ok(error.data && typeof error.data === "object", "Error data must be an object");
                                }
                                if (hasOwnProperty(error, "suggestions")) {
                                    assert.ok(typeof error.suggestions === "number" || error.suggestions === null || Array.isArray(error.suggestions), "Test error object property 'suggestions' should be an array or a number");
                                }
                                assert.ok(error.message || error.messageId, "Test error must specify either a 'messageId' or 'message'.");
                                assert.ok(!(error.message && error.messageId), "Error should not specify both 'message' and a 'messageId'.");
                                assert.ok(!(error.message && error.data), "Error should not specify both 'data' and 'message'.");
                                Object.keys(error).forEach(key => {
                                    assert.ok(errorObjectParameters.has(key), `Invalid error property name '${key}',Invalid test case error object should only have the following properties: ${friendlyErrorObjectParameterList}`);
                                });
                            } else {
                        ...
                        assert.fail("Invalid test cases should have an 'errors' property");
                    }
                }
            }
            ...
            // Ensure that the test cases are in the object-format.
            test.valid = test.valid.map(testCase => (typeof testCase === "string" ? { code: testCase } : testCase));

            test.valid.forEach(item => {
                assertTestCase(item, "valid");
                checkDuplicateTestCase(item, seenTestCases);
            });
            test.invalid.forEach(item => {
                assertTestCase(item, "invalid");
                checkDuplicateTestCase(item, seenTestCases);
            });
        ...
        function runRuleForItem(item) {
            ...
            configs.push({
                rules: {
                    [ruleId]: hasOwnProperty(item, "options") ? [1, ...item.options] : 1
                }
            });
            ...
            if (typeof item.errors === "number") {
                ...
                for (let i = 0, l = item.errors.length; i < l; i++) {
                    ...
                    if (hasOwnProperty(error, "suggestions")) {
                        ...
                        const expectsSuggestions = Array.isArray(error.suggestions) ? error.suggestions.length > 0 : error.suggestions > 0;
