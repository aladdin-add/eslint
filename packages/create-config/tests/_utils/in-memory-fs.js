...
const CLIEnginePath =
    require.resolve("eslint/lib/cli-engine/cli-engine");
const ConfigArrayFactoryPath =
    require.resolve("@eslint/eslintrc/lib/config-array-factory");
const FileEnumeratorPath =
    require.resolve("eslint/lib/cli-engine/file-enumerator");
const LoadRulesPath =
    require.resolve("eslint/lib/cli-engine/load-rules");
const ESLintPath =
    require.resolve("eslint/lib/eslint/eslint");
const ESLintAllPath =
    require.resolve("eslint/conf/eslint-all");
...
function defineConfigArrayFactoryWithInMemoryFileSystem({
    ...
    stubs["eslint/lib/shared/relative-module-resolver"] = RelativeModuleResolver;
...
function defineCLIEngineWithInMemoryFileSystem({
    ...
    const { CLIEngine, getCLIEngineInternalSlots } = proxyquire(CLIEnginePath, {
        ...
        "eslint/lib/shared/relative-module-resolver": RelativeModuleResolver
    });
...
function defineESLintWithInMemoryFileSystem({
    ...
    const { ESLint, getESLintPrivateMembers } = proxyquire(ESLintPath, {
        "eslint/lib/cli-engine/cli-engine": { CLIEngine, getCLIEngineInternalSlots }
    });
